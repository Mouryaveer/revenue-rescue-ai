...
from app.models.enums import ActorType, AuditEventType, RecoveryCaseStatus, RecoveryScenario
...
DEMO_CASES = [
    ...
    (RecoveryScenario.FAILED_PAYMENT,       "EXPIRED_METHOD",      299900,   "ESCALATED",  False, "[SYNTHETIC] Card expired — manual update needed"),
    (RecoveryScenario.FAILED_PAYMENT,       "BANK_DECLINE",        899900,   "ESCALATED",  False, "[SYNTHETIC] 3 retries exhausted"),
    (RecoveryScenario.FAILED_PAYMENT,       "AUTH_FAILURE",        149900,   "WAITING",    False, "[SYNTHETIC] Reminder sent — awaiting action"),
    (RecoveryScenario.FAILED_SUBSCRIPTION,  "MANDATE_FAILURE",     99900,    "RECOVERED",  True,  "[SYNTHETIC] Mandate retry success"),
    (RecoveryScenario.FAILED_SUBSCRIPTION,  "INSUFFICIENT_FUNDS",  199900,   "RECOVERED",  True,  "[SYNTHETIC] Subscription retry success"),
    (RecoveryScenario.CHECKOUT_ABANDONMENT, "CHECKOUT_ABANDONED",  349900,   "RECOVERED",  True,  "[SYNTHETIC] Recovery message → customer resumed"),
    (RecoveryScenario.CHECKOUT_ABANDONMENT, "CHECKOUT_ABANDONED",  249900,   "STOPPED",    False, "[SYNTHETIC] Opted-out → DENIED by policy"),
    (RecoveryScenario.CHECKOUT_ABANDONMENT, "CHECKOUT_ABANDONED",  499900,   "RECOVERED",  True,  "[SYNTHETIC] 2nd message → customer resumed"),
    (RecoveryScenario.FAILED_PAYMENT,       "BANK_DECLINE",        6000000,  "ESCALATED",  False, "[SYNTHETIC] ₹60,000 exceeds auto-recovery limit"),
    (RecoveryScenario.FAILED_PAYMENT,       "INSUFFICIENT_FUNDS",  299900,   "ESCALATED",  False, "[SYNTHETIC] RED-TEAM: 4th retry → POLICY DENIED"),
    (RecoveryScenario.FAILED_PAYMENT,       "INSUFFICIENT_FUNDS",  199900,   "STOPPED",    False, "[SYNTHETIC] RED-TEAM: opted-out → DENIED"),
    (RecoveryScenario.FAILED_PAYMENT,       "GATEWAY_TEMPORARY",   99900,    "RECOVERED",  True,  "[SYNTHETIC] Idempotency demo — already recovered"),
    (RecoveryScenario.FAILED_PAYMENT,       "UNKNOWN",             399900,   "ESCALATED",  False, "[SYNTHETIC] UNKNOWN failure → human review"),
    (RecoveryScenario.FAILED_PAYMENT,       "AUTH_FAILURE",        599900,   "ESCALATED",  False, "[SYNTHETIC] Suspicious activity → automation halted"),
    (RecoveryScenario.FAILED_PAYMENT,       "INSUFFICIENT_FUNDS",  149900,   "RECOVERED",  True,  "[SYNTHETIC] Quick recovery — retry #1 success"),
    (RecoveryScenario.FAILED_SUBSCRIPTION,  "SUBSCRIPTION_GRACE",  299900,   "RECOVERED",  True,  "[SYNTHETIC] Grace period retry success"),
    (RecoveryScenario.CHECKOUT_ABANDONMENT, "CHECKOUT_ABANDONED",  89900,    "STOPPED",    False, "[SYNTHETIC] Timeout not reached → DENIED"),
    (RecoveryScenario.FAILED_PAYMENT,       "BANK_DECLINE",        399900,   "FAILED",     False, "[SYNTHETIC] All retries exhausted — genuine failure"),
]
...
async def seed() -> None:
    ...
    async with session_factory() as session:
        ...
        for i, (scenario, failure_reason, amount, status, is_recovered, desc) in enumerate(DEMO_CASES):
            ...
            case = RecoveryCase(
                ...
                diagnosis={
                    ...
                    "notes": "DEMO_SEED",
                } if status != "DETECTED" else None,
                policy_decision="APPROVED" if is_recovered else (
                    "ESCALATE" if status == "ESCALATED" else
                    "DENIED"   if status == "STOPPED" else None
                ),
            )
            session.add(case)
            await session.flush()
            ...
            if status in ("ESCALATED", "STOPPED"):
                session.add(AuditEvent(
                    recovery_case_id=case_id,
                    event_type=AuditEventType.POLICY_DENIED if status == "STOPPED" else AuditEventType.POLICY_ESCALATE,
        ...
        print(f"  All data is SYNTHETIC — no real payments.")
